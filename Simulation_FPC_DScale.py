...
class Simulation:
    def __init__(self, args, truth, truth_like, record=False):
        ...
        self.popularity = np.zeros(self.num_item)
    ...
    def initial_iterations(self):
        ...
        if self.record:
            np.save('./Data/' + self.data + '/Experiment_FPC_DScale/init_click_mat.npy', init_click_mat)
    ...
    def run_simulation(self):
        ...
        for c in range(int(self.iteration / self.cycle_itr)):
            for _ in tqdm(range(self.cycle_itr)):
                ...
                ''' popularity scaling '''
                alpha = 0. + (c + 1) / (self.iteration / self.cycle_itr) * self.alpha
                weights = (1. / (self.popularity + 1.)) ** alpha
                weights *= self.num_item / np.sum(weights)
                scores *= weights
        ...
        if self.record:
            np.save('./Data/' + self.data + '/Experiment_FPC_DScale/itr_user.npy', np.array(itr_user))
            np.save('./Data/' + self.data + '/Experiment_FPC_DScale/itr_rec_item.npy', np.array(itr_rec_item))
            np.save('./Data/' + self.data + '/Experiment_FPC_DScale/itr_click_item.npy', np.array(itr_click_item, dtype=object))
