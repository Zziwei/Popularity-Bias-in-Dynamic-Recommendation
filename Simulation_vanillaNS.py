...
from MF_vanillaNS import MF_vanillaNS
...
class Simulation:
    ...
    def initial_iterations(self):
        ...
        if self.record:
            np.save('./Data/' + self.data + '/Experiment_vanillaNS/init_click_mat.npy', init_click_mat)
    ...
    def run_simulation(self):
        ...
        mf = MF_vanillaNS(self.lr, self.reg, self.hidden, self.neg, self.num_user, self.num_item)
        feedback_array = np.array(self.feedback)
        neg_feedback_array = np.array(self.neg_feedback)
        for j in range(self.epoch):
            mf.train(feedback_array, j, bs, verbose=True)
            mf.record()
        ...
        for c in range(int(self.iteration / self.cycle_itr)):
            ...
            for j in tqdm(range(self.epoch)):
                mf.train(feedback_array, j, bs, verbose=False)
            mf.record()
            print('')

        if self.record:
            np.save('./Data/' + self.data + '/Experiment_vanillaNS/itr_user.npy', np.array(itr_user))
            np.save('./Data/' + self.data + '/Experiment_vanillaNS/itr_rec_item.npy', np.array(itr_rec_item))
            np.save('./Data/' + self.data + '/Experiment_vanillaNS/itr_click_item.npy', np.array(itr_click_item, dtype=object))
