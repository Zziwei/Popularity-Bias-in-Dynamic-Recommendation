...
class Simulation:
    ...
    def initial_iterations(self):
            ...
            for u in tqdm(range(self.num_user)):
                ...
                for k in range(self.K):
                    ...
                    if iid in u_truth_like and randomK[k] <= utility.position_bias[k]:
                        ...
                        self.random_feedback.append([u, iid, k])
                        init_click_mat[u, iid] = 1.
                        self.truth_unclick[u, iid] = 0
                    else:
                        self.random_neg_feedback.append([u, iid, k])
        ...
        if self.record:
            np.save('./Data/' + self.data + '/Experiment_withoutCFL/init_click_mat.npy', init_click_mat)
    ...
    def run_simulation(self):
        ...
        bs = int(len(self.random_feedback) * (self.neg + 1) / 50.)
        print('Update bs to ' + str(bs))
        mf = MF_withoutPB(self.lr, self.reg, self.hidden, self.neg, self.num_user, self.num_item)
        random_feedback_array = np.array(self.random_feedback)
        random_neg_feedback_array = np.array(self.random_neg_feedback)
        for j in range(self.epoch):
            mf.train(random_feedback_array, random_neg_feedback_array, j, bs, verbose=True)
            mf.record()
        ...
        for c in range(int(self.iteration / self.cycle_itr)):
            ...
            ''' Do the random recommendation process to collect randomly exposed feedback '''
            tmp_random_click_item = np.zeros(self.num_item)
            random_users = np.random.randint(self.num_user, size=self.cycle_itr * 1)
            ...
            print('#' * 10
                  ...
                  % (itr, len(self.feedback) - len(self.random_feedback), gc, time.time() - last_time)
                  + '#' * 10)
            ...
            for j in tqdm(range(self.epoch)):
                mf.train(random_feedback_array, random_neg_feedback_array, j, bs,  verbose=False)
            mf.record()
            print('')

        if self.record:
            np.save('./Data/' + self.data + '/Experiment_withoutCFL/itr_user.npy', np.array(itr_user))
            np.save('./Data/' + self.data + '/Experiment_withoutCFL/itr_rec_item.npy', np.array(itr_rec_item))
            np.save('./Data/' + self.data + '/Experiment_withoutCFL/itr_click_item.npy', np.array(itr_click_item, dtype=object))
